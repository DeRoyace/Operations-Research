...
// of a transportation table using Matrix Minima Method.
...
int compute()
{
    // code to compute Transportation problem using Matrix Minima method:
    ...
    while(sum(1) !=0 && sum(0) != 0)     // executing the loop until all requirements and capacity cells are 0
    {
        ...
        for(i = 0; i < row-1; i++)
        {
            for(j = 0; j < col-1; j++)
            {
                if(minCost > arr[i][j] && arr[i][col-1] != 0 && arr[row-1][j] != 0)
                {
                    minCost = arr[i][j];
                    row_index = i;
                    col_index = j;
